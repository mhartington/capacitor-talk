(function(ionic) {
  ...
  ionic.views.ScrollNative = ionic.views.View.inherit({
    initialize: function(options) {
      ...
      self.options = {
        ...
        getContentHeight: function() {
          return Math.max(
            self.__content.scrollHeight,
            self.__content.offsetHeight + self.__content.offsetTop * 2
          );
        }
      };
    ...
    __computeScrollMax: function() {
      ...
      if (
        !self.__didWaitForSize &&
        !self.__maxScrollLeft &&
        !self.__maxScrollTop
      ) {
    ...
    __initEventHandlers: function() {
      ...
      self.scrollChildIntoView = function(e) {
        ...
        var changedKeyboardHeight =
          lastKeyboardHeight && lastKeyboardHeight !== e.detail.keyboardHeight;
        ...
        if (!alreadyShrunk || changedKeyboardHeight) {
          ...
          if (
            ...
            ionic.requestAnimationFrame(function() {
              ...
              scrollViewOffsetHeight = Math.max(
                0,
                Math.min(
                  self.__originalContainerHeight,
                  self.__originalContainerHeight -
                    (e.detail.keyboardHeight - 43)
                )
              ); //keyboardOffset >= 0 ? scrollViewOffsetHeight - keyboardOffset : scrollViewOffsetHeight + keyboardOffset;
    ...
    __cleanup: function() {
      ...
      container.removeEventListener(
        ionic.EVENTS.touchstart,
        self.handleTouchMove
      );
      container.removeEventListener(
        ionic.EVENTS.touchmove,
        self.handleTouchMove
      );
